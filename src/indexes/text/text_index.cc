...
TextIndex::TextIndex(bool with_suffix)
    : suffix_tree_(
          with_suffix ? std::make_unique<RadixTree<std::shared_ptr<Postings>>>()
                      : nullptr) {}
      ...
      // the key and position map
      {
        ...
        updated_target =
            text_index_->GetPrefix().MutateTarget(token, [&](auto existing) {
              ...
              // Note: Right now this won't include the position map memory
              // since it's already allocated and moved into the postings
              // object. Once we start creating a serialized version instead
              // then it will be tracked. At that point stop moving the pos_map
              // and just pass a reference so that it doesn't get cleaned up in
              // the memory scope.
